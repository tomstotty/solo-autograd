"""Contract tests for Tensor.conv_transpose1d_batch transposed convolution.
...
W = [float(v) / 10.0 for v in range(1, 13)]   # length C*O*K = 12
...
def _reference(x, w, B, C, O, N, K, S, P, D, grad=None):
    ...
    dw = [0.0] * (C * O * K) if grad is not None else None
    for b in range(B):
        for c in range(C):
            for i in range(N):
                xi = (b * C + c) * N + i
                for o in range(O):
                    ...
                    for r in range(K):
                        ...
                        y[oi] += x[xi] * w[wi + r]
                        if grad is not None:
                            dx[xi] += grad[oi] * w[wi + r]
                            dw[wi + r] += grad[oi] * x[xi]
    if grad is None:
        return y
    return y, dx, dw, L


class ConvTranspose1dBatchForwardTests(unittest.TestCase):
    def test_matches_reference(self):
        out = Tensor(X).conv_transpose1d_batch(Tensor(W), 2, 2, 2, 4, 3)
        ref = _reference(X, W, 2, 2, 2, 4, 3, 1, 0, 1)
        self.assertEqual(out.data, ref)
        # L = (4 - 1) + 2 + 1 = 6
        self.assertEqual(len(out.data), 2 * 2 * 6)

    def test_output_length_formula(self):
        # N=5, K=2, S=2, P=1, D=2 -> 4*2 - 2 + 2 + 1 = 9
        out = Tensor([0.0] * 5).conv_transpose1d_batch(
    ...
    def test_stride_dilation_padding_values(self):
        ...
        w = Tensor([2.0, 3.0])
        out = x.conv_transpose1d_batch(
            w, 1, 1, 1, 5, 2, stride=2, padding=1, dilation=2
        )
        # t = 2i - 1 + 2r; only the odd positions are ever hit.
        self.assertEqual(
            out.data, [0.0, 7.0, 0.0, 12.0, 0.0, 17.0, 0.0, 22.0, 0.0]
        )

    def test_accumulation_runs_from_zero_in_ascending_order(self):
        # L=1 with N=2, K=2, P=1: t=i-1+r. The (i=0, r=1) product lands
        # first and is 1e16, swallowing the later (i=1, r=0) add of 1.0,
        # pinning the mandated (b, c, i, o, r) order.
        x = [1e16, 1.0]
        out = Tensor(x).conv_transpose1d_batch(
            Tensor([1.0, 1.0]), 1, 1, 1, 2, 2, padding=1
        )
    ...
    def test_graph_presence(self):
        ...
        out = Tensor(X, True).conv_transpose1d_batch(
            Tensor(W), 2, 2, 2, 4, 3
        )
    ...
    def test_forward_snapshots_both_operands(self):
        ...
        out = x.conv_transpose1d_batch(w, 1, 1, 1, 4, 2)
        saved = list(out.data)
        x.data = [100.0] * 4
        w.data = [0.0, 0.0]
        self.assertEqual(out.data, saved)
        out.backward([1.0] * 5)
        self.assertEqual(x.grad, [3.0, 3.0, 3.0, 3.0])
        self.assertEqual(w.grad, [10.0, 10.0])


class ConvTranspose1dBatchValidationTests(unittest.TestCase):
    def test_kernel_must_be_tensor(self):
        expect(
            TypeError,
            lambda: Tensor([1.0] * 4).conv_transpose1d_batch(
                [1.0, 1.0], 1, 1, 1, 4, 2),
        )
    ...
    def test_data_must_be_nonempty_finite_float_vectors(self):
        ...
        expect(ValueError,
               lambda: _raw(1.0, False).conv_transpose1d_batch(
                   good_w, 1, 1, 1, 4, 2))
        expect(ValueError,
               lambda: _raw([], False).conv_transpose1d_batch(
                   good_w, 1, 1, 1, 4, 2))
        expect(TypeError,
               lambda: _raw([1] * 4, False).conv_transpose1d_batch(
                   good_w, 1, 1, 1, 4, 2))
        expect(TypeError,
               lambda: _raw([1.0, True, 1.0, 1.0], False).conv_transpose1d_batch(
                   good_w, 1, 1, 1, 4, 2))
        good_x = Tensor([1.0] * 4)
        expect(ValueError,
               lambda: good_x.conv_transpose1d_batch(
                   _raw([], False), 1, 1, 1, 4, 2))
        expect(ValueError,
               lambda: good_x.conv_transpose1d_batch(
    ...
    def test_requires_grad_must_be_bool(self):
        ...
        expect(TypeError,
               lambda: bad.conv_transpose1d_batch(
                   Tensor([1.0, 1.0]), 1, 1, 1, 4, 2))
        ...
        for i, name in enumerate(names):
            for bad in (True, 1.0, "1", None, [1]):
                ...
                with self.subTest(name, bad=bad):
                    expect(TypeError,
                           lambda args=args:
                               x.conv_transpose1d_batch(w, *args))
            if name == "padding":
    ...
    def test_padding_must_be_non_bool_non_negative_int(self):
        ...
        for bad in (True, False, 1.0, "0", None):
            expect(TypeError,
                   lambda bad=bad: x.conv_transpose1d_batch(
                       w, 1, 1, 1, 4, 2, padding=bad))
        expect(ValueError,
               lambda: x.conv_transpose1d_batch(
                   w, 1, 1, 1, 4, 2, padding=-1))

    def test_data_lengths_must_match_shapes(self):
        x, w = Tensor([1.0] * 4), Tensor([1.0, 1.0])
        # B*C*N = 8 but x has 4
        expect(ValueError,
               lambda: x.conv_transpose1d_batch(w, 2, 1, 1, 4, 2))
        # C*O*K = 3 but w has 2
        expect(ValueError,
               lambda: x.conv_transpose1d_batch(
                   Tensor([1.0, 1.0]), 1, 1, 1, 4, 3))

    def test_output_length_must_be_positive(self):
        x, w = Tensor([1.0] * 4), Tensor([1.0, 1.0])
        # N=4, K=2, S=1, P=3, D=1 -> L = 3 - 6 + 1 + 1 = -1
        expect(ValueError,
               lambda: x.conv_transpose1d_batch(
                   w, 1, 1, 1, 4, 2, padding=3))
    ...
    def test_nonfinite_forward_is_value_error(self):
        ...
        expect(ValueError,
               lambda: x.conv_transpose1d_batch(
                   Tensor([1.0, 1.0]), 1, 1, 1, 4, 2))
        huge = sys_float_max()
        # y[1] receives huge from both taps and overflows to inf.
...
class ConvTranspose1dBatchBackwardTests(unittest.TestCase):
    def test_gradients_match_reference(self):
        B, C, O, N, K, S, P, D = 2, 2, 2, 4, 3, 1, 0, 1
        x = Tensor(X, True)
        w = Tensor(W, True)
        out = x.conv_transpose1d_batch(w, B, C, O, N, K, stride=S,
                                       padding=P, dilation=D)
        L = (N - 1) * S - 2 * P + D * (K - 1) + 1
        g = [((i % 5) + 1) * 0.5 for i in range(B * O * L)]
        out.backward(list(g))
    ...
    def test_dilated_strided_padded_gradients(self):
        ...
        out.backward([1.0] * 9)
        self.assertEqual(x.grad, [3.0, 5.0, 5.0, 5.0, 2.0])
        self.assertEqual(w.grad, [14.0, 10.0])

    def test_dw_reduces_across_batch(self):
        x = Tensor([1.0, 2.0, 1.0, 2.0], True)
        w = Tensor([1.0, 1.0], True)
        out = x.conv_transpose1d_batch(w, 2, 1, 1, 2, 2)
        self.assertEqual(out.data, [1.0, 3.0, 2.0, 1.0, 3.0, 2.0])
        out.backward([1.0] * 6)
    ...
    def test_only_parents_requiring_grad_receive_contributions(self):
        ...
        x.conv_transpose1d_batch(w, 1, 1, 1, 4, 2).backward([1.0] * 5)
        self.assertEqual(x.grad, [3.0, 3.0, 3.0, 3.0])
    ...
    def test_aliased_parents_merge_roles(self):
        ...
        out = z.conv_transpose1d_batch(z, 1, 1, 1, 4, 4)
        self.assertEqual(
            out.data, [1.0, 4.0, 10.0, 20.0, 25.0, 24.0, 16.0]
        )
        out.backward([1.0] * 7)
        # With g all ones, every dx[i] sums z[r] = 1+2+3+4 = 10 and every
        # dw[r] sums z[i] = 1+2+3+4 = 10; the two roles merge per element.
        self.assertEqual(z.grad, [20.0, 20.0, 20.0, 20.0])
    ...
    def test_repeated_backward_accumulates(self):
        ...
        out.backward([1.0] * 5)
        out.backward([1.0] * 5)
        self.assertEqual(x.grad, [6.0, 6.0, 6.0, 6.0])
        self.assertEqual(w.grad, [20.0, 20.0])

    def test_grad_validation(self):
        out = Tensor([1.0] * 4, True).conv_transpose1d_batch(
            Tensor([1.0, 1.0], True), 1, 1, 1, 4, 2
        )
        # output length 5: omitted and scalar/bool/int/float -> ValueError,
        # wrong length or non-finite element -> ValueError
        for bad in ([], True, False, 1, 1.0, [1.0] * 4,
                    [1.0] * 4 + [float("nan")]):
        ...
        expect(TypeError, lambda: out.backward([1] * 5))
    ...
    def test_nonfinite_backward_is_atomic_failure(self):
        ...
        out = x.conv_transpose1d_batch(w, 1, 1, 1, 4, 2)
        expect(ValueError, lambda: out.backward([huge, 0.0, 0.0, 0.0, 0.0]))
        self.assertIsNone(x.grad)
        self.assertIsNone(w.grad)
    ...
    def test_gradcheck_input(self):
        ...
        passed, error = gradcheck(
            lambda d: d.conv_transpose1d_batch(
                w, 2, 2, 2, 4, 3).sum(),
    ...
    def test_gradcheck_weight(self):
        x = Tensor(X, False)
        passed, error = gradcheck(
            lambda d: x.conv_transpose1d_batch(
                d, 2, 2, 2, 4, 3).sum(),
...
class ConvTranspose1dBatchCliRegressionTests(unittest.TestCase):
